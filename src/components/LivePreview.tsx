...
const LivePreview: React.FC<Props> = ({ activeUseCase }) => {
  ...
  const isActionMode = activeUseCase.id.includes('fitting') || activeUseCase.id.includes('quality') || activeUseCase.id.includes('stylist') || activeUseCase.id.includes('sustainable');
  ...
  const vizLabels: Record<string, { idle: string; running: string; done: string }> = {
    trend:       { idle: 'トレンドデータ待機',           running: 'トレンド予測実行中',           done: 'トレンド予測完了' },
    inventory:   { idle: '在庫データ待機',               running: '在庫最適化実行中',             done: '在庫最適化完了' },
    supplychain: { idle: 'サプライチェーン待機',         running: 'SCM最適化実行中',              done: 'SCM最適化完了' },
    material:    { idle: '素材データ待機',               running: '素材配合最適化中',             done: '素材配合完了' },
    size:        { idle: '体型データ待機',               running: 'サイズ推薦実行中',             done: 'サイズ推薦完了' },
    fitting:     { idle: '3Dスキャン待機',               running: 'バーチャル試着実行中',         done: 'フィッティング完了' },
    design:      { idle: 'デザインAI待機',               running: 'デザイン生成実行中',           done: 'デザイン生成完了' },
    production:  { idle: '生産データ待機',               running: 'ロット最適化実行中',           done: 'ロット最適化完了' },
    color:       { idle: 'カラーデータ待機',             running: 'パレット最適化中',             done: 'パレット最適化完了' },
    runway:      { idle: 'ショー構成待機',               running: 'ショー最適化実行中',           done: 'ショー最適化完了' },
    sustainable: { idle: 'サステナデータ待機',           running: '環境負荷計算中',               done: '環境最適化完了' },
    stylist:     { idle: 'スタイル分析待機',             running: 'コーデAI実行中',               done: 'コーデ提案完了' },
    textile:     { idle: 'テキスタイル待機',             running: 'テキスタイル生成中',           done: 'テキスタイル完了' },
    fairtrade:   { idle: '取引データ待機',               running: 'フェアトレード解析中',         done: '公正取引確認完了' },
    luxury:      { idle: 'VIPデータ待機',                running: '需要予測実行中',               done: '需要予測完了' },
    logistics:   { idle: '物流データ待機',               running: '配送最適化実行中',             done: '配送最適化完了' },
    marketing:   { idle: 'マーケデータ待機',             running: 'CVR最適化実行中',              done: 'CVR最適化完了' },
    quality:     { idle: '検査データ待機',               running: 'AI品質検査実行中',             done: 'AI品質検査完了' },
    collection:  { idle: 'コレクション待機',             running: 'コレクション最適化中',         done: 'コレクション完了' },
    resale:      { idle: 'リセールデータ待機',           running: '価格予測実行中',               done: '価格予測完了' },
  };

  const getVizLabel = () => {
    const type = getVizType(activeUseCase.id);
    const labels = vizLabels[type] || vizLabels.trend;
